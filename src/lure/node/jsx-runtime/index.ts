 ...
 * Reason for changes: Keep style=S`...` out of attributes (else-if chain was
 * broken so style also fell into attributes and bypassed reflectStyles).
 */
...
export const createElement = (type: string | HTMLElement | Node | DocumentFragment | Document | Element | Function | symbol, props: any = {}, children?: any[]|any|null, ...others: any[]|any|null)=>{
    ...
    for (const i in props) {
        if (i == 'ref') {
            ...
            // INVARIANT: S`...` / css`...` / style objects stay on params.style → reflectStyles.
            style = props[i];
        } else if (i?.startsWith?.("@")) {
            const name = i.replace("@", "").trim();
            if (name) { bindEvent(on, name, props[i]); } else { on = props[i]; }
        } else if (i?.startsWith?.("on:")) {
            const name = i.replace("on:", "").trim();
            if (name) { bindEvent(on, name, props[i]); } else { on = props[i]; }
        } else if (i?.startsWith?.("prop:")) {
            const name = i.replace("prop:", "").trim();
            if (name) { properties[name] = props[i]; } else { properties = props[i]; }
        } else if (i?.startsWith?.("attr:")) {
            const name = i.replace("attr:", "").trim();
            if (name) { attributes[name] = props[i]; } else { attributes = props[i]; }
        } else if (i?.startsWith?.("ctrl:")) {
            const name = i.replace("ctrl:", "").trim();
            if (name) { ctrls.set(name, props[i]); } else { ctrls = props[i]; }
        } else if (i !== "children" && i !== "key") {
