/*
 * Filename: run-mapped-lifecycle-tests.mjs
 * FullPath: modules/projects/lur.e/scripts/run-mapped-lifecycle-tests.mjs
 * FIND:style-anim
 * WHY: Bundle and run Utils append/remove lifecycle waits with Node's native test runner.
 */
...
const outDir = resolve(root, ".tmp-tests/mapped-lifecycle");
const entry = resolve(root, "test/mapped-lifecycle.node.test.ts");
const output = resolve(outDir, "mapped-lifecycle.node.test.mjs");
...
await build({
    ...
    build: {
        ...
        lib: {
            entry,
            formats: ["es"],
            fileName: () => "mapped-lifecycle.node.test.mjs"
        },
        rollupOptions: {
            external: [/^node:/, "jsdom"],
            treeshake: {
