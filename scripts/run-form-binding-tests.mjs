...
const outDir = resolve(root, ".tmp-tests/form-binding");
const entry = resolve(root, "test/form-binding.node.test.ts");
const output = resolve(outDir, "form-binding.node.test.mjs");
...
await build({
    ...
    build: {
        ...
        lib: {
            entry,
            formats: ["es"],
            fileName: () => "form-binding.node.test.mjs",
        },
        rollupOptions: {
