...
const outDir = resolve(root, ".tmp-tests/placement");
const entry = resolve(root, "test/placement.node.test.ts");
const output = resolve(outDir, "placement.node.test.mjs");
...
await build({
    ...
    build: {
        ...
        lib: {
            entry,
            formats: ["es"],
            fileName: () => "placement.node.test.mjs",
        },
        rollupOptions: {
            treeshake: {
