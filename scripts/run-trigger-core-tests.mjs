...
const outDir = resolve(root, ".tmp-tests/trigger-core");
const entry = resolve(root, "test/trigger-core.node.test.ts");
const output = resolve(outDir, "trigger-core.node.test.mjs");
...
await build({
    ...
    resolve: {
        alias: [
            ...
            { find: "@fest-lib/dom", replacement: resolve(root, "test/stubs/dom.ts") },
    ...
    build: {
        ...
        lib: {
            entry,
            formats: ["es"],
            fileName: () => "trigger-core.node.test.mjs",
        },
        rollupOptions: {
            treeshake: {
