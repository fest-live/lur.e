...
const outDir = resolve(root, ".tmp-tests/interaction");
const entry = resolve(root, "test/interaction.node.test.ts");
const output = resolve(outDir, "interaction.node.test.mjs");
...
await build({
    ...
    build: {
        ...
        lib: {
            entry,
            formats: ["es"],
            fileName: () => "interaction.node.test.mjs",
        },
        rollupOptions: {
            treeshake: {
